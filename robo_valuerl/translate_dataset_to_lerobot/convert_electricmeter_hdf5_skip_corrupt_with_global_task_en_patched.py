...
BUILTIN_EN_TEXT_MAP = {
    ...
    # Subgoal variants / canonical labels
    ...
    "Place the joint into the good-part pile": "Place the joint into the good-part pile",
    "Place the joint into the good-part pile": "Place the joint into the good-part pile",
    "Place the joint into the defective-part pile": "Place the joint into the defective-part pile",
    "Place the joint into the defective-part pile": "Place the joint into the defective-part pile",

    # When a mixed label appears, normalize it to the good-part placement label as requested
    "Place the joint into the good-part pile": "Place the joint into the good-part pile",
...
def translate_to_english(text: str, translation_map: dict[str, str] | None = None) -> str:
    ...
    # Placement labels
    if "Place the joint into" in compact or "placed into the good part" in compact or "placed into the defective part" in compact:
        if "good part" in compact:
            return "Place the joint into the good-part pile"
        if "defective part" in compact:
    ...
    if "good part" in compact and "joint" in compact:
        return "Place the joint into the good-part pile"
    if "defective part" in compact and "joint" in compact:
        return "Place the joint into the defective-part pile"
...
def get_episode_key_from_any_path(path_like: str | Path) -> str:
    ...
    Matching order matters: keep the more explicit legacy success_episodes rules first,
    then add support for classified paths without breaking older datasets.
    """
    parts = split_path_parts(path_like)

    # Legacy / annotation layout: .../success_episodes/<episode_id>/...
    if "success_episodes" in parts:
        idx = parts.index("success_episodes")
        if idx + 1 < len(parts):
            return parts[idx + 1]

    # Legacy raw trajectory layout: .../<episode_id>/data/trajectory.hdf5
    if len(parts) >= 3 and parts[-1] == "trajectory.hdf5" and parts[-2] == "data":
        return parts[-3]

    # Classified trajectory layout: .../<episode_id>/trajectory.hdf5
    if len(parts) >= 2 and parts[-1] == "trajectory.hdf5":
        return parts[-2]

    # Directory layout: .../<episode_id>/data
...
def parse_frame_range(frame_str: str) -> tuple[int, int]:
    """
    Support both interval labels like "211,809" and single-frame labels like "1122".
    """
