...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use_subgoal_as_task",
        action="store_true",
        help="If enabled, each frame's task_index uses current subgoal_index and tasks.jsonl is written from subgoal vocab.",
    )
...
def main() -> int:
    ...
    for task_idx, (task_key, traj_list) in enumerate(task_groups.items()):
        ...
        lengths: List[int] = [read_num_steps(traj_path) for traj_path in traj_list]
        ...
        subgoal_vocab, subgoal_to_idx = collect_subgoal_vocab_for_trajs(traj_list, ann_index)
    ...
    for task_output_name, plan in task_plans.items():
        ...
        print(
            f"[INFO] Finished task [{plan['task_key']}] | ok={ok_count} | failed={fail_count} | output={dataset_output_dir}"
        )
    ...
    print(f"[INFO] Done | total_ok={total_ok} | total_failed={total_fail}")
    return 0
