...
def load_trajectory(trajectory_path, task_instruction):
    try:
        ...
        if "y_pressed" in data.keys():
            ...
            y_pressed_cumsum = np.zeros((timesteps))
            y_pressed = np.zeros((timesteps))
            y_pressed = y_pressed * 50
            y_pressed_cumsum = np.cumsum(y_pressed)
        ...
        cam_key = None
        if 'camera_head' in data['camera_observations']['color_images']:
            cam_key = 'camera_head'
        elif 'camera_top' in data['camera_observations']['color_images']:
            cam_key = 'camera_top'
        else:
            # Raise an error (or apply a default) if neither exists
            raise KeyError("Neither 'camera_head' nor 'camera_top' found in color_images.")

        for step in range(timesteps):
            # Image processing and resize
            head_image = CameraUtils.decode_color_image(data['camera_observations']['color_images'][cam_key][step], input_format="bgr", output_format="rgb")
        ...
        for type_name in os.listdir(args.raw_data_dir):
            ...
            if os.path.isdir(type_path):
                for file_name in os.listdir(type_path):
                    data_path = os.path.join(type_path, file_name, "data","trajectory.hdf5")
