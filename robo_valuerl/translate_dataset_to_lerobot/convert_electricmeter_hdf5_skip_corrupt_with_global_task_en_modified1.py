...
def setup_output_logging(log_path: Path):
    ...
    return log_fp, original_stdout, original_stderr
...
DEFAULT_TRANSLATION_JSON_NAME = "electricmeter_translation_map.json"
IGNORED_ANNOTATION_LABELS = {"high quality", "average", "poor quality"}
IGNORED_ANNOTATION_LABELS_NORMALIZED = {re.sub(r"\s+", "", str(x).strip().lower()) for x in IGNORED_ANNOTATION_LABELS}
...
def load_translation_map(translation_json_path: str | None) -> tuple[dict[str, str], Path]:
    if translation_json_path:
        path = Path(translation_json_path).expanduser().resolve()
    else:
        path = Path(__file__).resolve().with_name(DEFAULT_TRANSLATION_JSON_NAME)

    if not path.exists():
        raise FileNotFoundError(
            f"translation json does not exist: {path}. "
            f"Please provide --translation_json or place {DEFAULT_TRANSLATION_JSON_NAME} next to the script."
        )
    ...
    if isinstance(obj, dict):
        ...
        for item in obj:
            if not isinstance(item, dict):
                continue
            src_text = str(item.get("src", item.get("zh", item.get("source", "")))).strip()
            dst_text = str(item.get("dst", item.get("en", item.get("target", "")))).strip()
            if src_text:
                mapping[src_text] = dst_text
    else:
        raise ValueError("translation json must be a dict or a list of {src,dst} objects")

    return mapping, path


def should_ignore_annotation_label(text: str) -> bool:
    return normalize_text_key(text) in IGNORED_ANNOTATION_LABELS_NORMALIZED


def translate_to_english(text: str, translation_map: dict[str, str] | None = None) -> str:
    s = str(text).strip()
    if not s:
        return s

    mapping = translation_map if translation_map is not None else {}
    ...
    normalized_lookup = normalize_text_key(s)
    normalized_mapping = {normalize_text_key(k): str(v) for k, v in mapping.items()}
    if normalized_lookup in normalized_mapping:
        return normalized_mapping[normalized_lookup]

    # 3) Heuristic fallback for common annotation variants / truncated strings
    compact = re.sub(r"[\s\-_/()]+", "", s)

    if compact == "<none>" or compact == "none":
        return "<none>"

    # Global task
    if "Inspect joint quality and separate good parts from defective parts" in compact:
        return "Inspect joint quality and separate good parts from defective parts"

    # Pick up uninspected joint
    if compact.startswith(("Pick up", "Pick up")) and "uninspected" in compact:
        return "Pick up the uninspected joint"

    # Inspect quality
    if "Inspect the joint quality" in compact:
        return "Inspect the joint quality"

    # Observe passage
    if ("Observe" in compact or "Observe" in compact) and "joint passage" in compact:
        return "Observe the joint passage"

    # Placement labels
    if "Place the joint into" in compact or "placed into the good part" in compact or "placed into the defective part" in compact or "placed into the defective part" in compact:
        if "good part" in compact:
            return "Place the joint into the good-part pile"
        if "defective part" in compact or "defective part" in compact:
            return "Place the joint into the defective-part pile"

    # Additional loose fallback for truncated labels
    if "uninspected" in compact:
        return "Pick up the uninspected joint"
    if "good part" in compact and "joint" in compact:
        return "Place the joint into the good-part pile"
    if ("defective part" in compact or "defective part" in compact) and "joint" in compact:
        return "Place the joint into the defective-part pile"
...
def task_key_to_instruction(task_key: str) -> str:
    text = str(task_key).replace("/", " ").replace("_", " ").replace("-", " ").strip()
    text = re.sub(r"\s+", " ", text)
    return text


def resolve_global_task_instruction(task_key: str, translation_map: dict[str, str] | None = None) -> str:
    normalized = str(task_key).lower().replace("_", "").replace("-", "").replace("/", "").replace(" ", "")
    if "electricmeter" in normalized:
        return translate_to_english("Inspect joint quality and separate good parts from defective parts", translation_map)
    return translate_to_english(task_key_to_instruction(task_key), translation_map)
...
def read_num_steps(trajectory_path: Path) -> int:
    with h5py.File(trajectory_path, "r") as data:
        ...
        sequence_lengths = collect_required_sequence_lengths(data, resolved_cameras)
        effective_timesteps = min(sequence_lengths.values())
        if effective_timesteps <= 0:
            raise ValueError(f"Empty trajectory after alignment: {trajectory_path}")
        return int(effective_timesteps)
...
def get_episode_key_from_any_path(path_like: str | Path) -> str:
    """
    Extract episode id such as 0326_150036 from any of these common layouts:
    - annotation rawDataPath .../success_episodes/<episode_id>/data
    - raw hdf5 path       .../success_episodes/<episode_id>/data/trajectory.hdf5
    - classified hdf5     .../<task_name>/<episode_id>/trajectory.hdf5
    - fallback directory  .../<episode_id>/data
    """
    ...
    # Classified trajectory layout: .../<task_name>/<episode_id>/trajectory.hdf5
    if len(parts) >= 2 and parts[-1] == "trajectory.hdf5":
        return parts[-2]
...
def load_annotation_index(annotation_json_path: str | None) -> dict[str, dict]:
    ...
    for json_file in json_files:
        ...
        episode_labels = obj.get("episodeLabels")
        if not isinstance(episode_labels, list):
            data = obj.get("data")
            if isinstance(data, dict):
                episode_labels = data.get("episodeLabels")

        if not isinstance(episode_labels, list):
            print(f"[WARN] Skip annotation file without episodeLabels list: {json_file}")
            continue
...
def collect_subgoal_vocab_for_trajs(
    traj_list: list[Path],
    ann_index: dict[str, dict],
    ...
    for traj in traj_list:
        ...
        for item in ann.get("labels", []):
            ...
            if should_ignore_annotation_label(raw_label):
                continue
            label = translate_to_english(raw_label, translation_map)
...
def build_framewise_subgoal(
    ...
    for item in annotation_record.get("labels", []):
        raw_label_name = str(item.get("label", "")).strip()
        if should_ignore_annotation_label(raw_label_name):
            continue
        label_name = translate_to_english(raw_label_name, translation_map)
...
def dataset_num_frames(dataset: h5py.Dataset) -> int:
    if len(dataset.shape) == 0:
        raise ValueError("Scalar dataset does not have a frame dimension")
    return int(dataset.shape[0])


def collect_required_sequence_lengths(
    data: h5py.File,
    resolved_cameras: dict[str, tuple[h5py.Dataset, str]] | None = None,
) -> dict[str, int]:
    if resolved_cameras is None:
        resolved_cameras = resolve_camera_datasets(data)

    lengths = {
        "master.arm_left_position_align": dataset_num_frames(data["master"]["arm_left_position_align"]["data"]),
        "master.arm_right_position_align": dataset_num_frames(data["master"]["arm_right_position_align"]["data"]),
        "master.end_effector_left_position_align": dataset_num_frames(
            data["master"]["end_effector_left_position_align"]["data"]
        ),
        "master.end_effector_right_position_align": dataset_num_frames(
            data["master"]["end_effector_right_position_align"]["data"]
        ),
        "puppet.arm_left_position_align": dataset_num_frames(data["puppet"]["arm_left_position_align"]["data"]),
        "puppet.arm_right_position_align": dataset_num_frames(data["puppet"]["arm_right_position_align"]["data"]),
        "camera.main": dataset_num_frames(resolved_cameras["main"][0]),
        "camera.left": dataset_num_frames(resolved_cameras["left"][0]),
        "camera.right": dataset_num_frames(resolved_cameras["right"][0]),
    }

    puppet_group = data["puppet"]
    if "end_effector_left_position_align" in puppet_group.keys():
        lengths["puppet.end_effector_left_position_align"] = dataset_num_frames(
            puppet_group["end_effector_left_position_align"]["data"]
        )
    if "end_effector_right_position_align" in puppet_group.keys():
        lengths["puppet.end_effector_right_position_align"] = dataset_num_frames(
            puppet_group["end_effector_right_position_align"]["data"]
        )
    if "y_pressed" in data.keys():
        lengths["y_pressed"] = dataset_num_frames(data["y_pressed"])
...
def convert_one_episode(job: dict) -> dict:
    ...
    translation_map = job.get("translation_map", None)
    ...
    try:
        with h5py.File(trajectory_path, "r") as data:
            resolved_cameras = resolve_camera_datasets(data)
            sequence_lengths = collect_required_sequence_lengths(data, resolved_cameras)
            raw_timesteps = int(sequence_lengths["master.arm_left_position_align"])
            timesteps = min(sequence_lengths.values())
            if timesteps <= 0:
                raise ValueError(f"Empty trajectory after alignment: {trajectory_path}")
            ...
            if "y_pressed" in data.keys():
                y_pressed = data["y_pressed"][:timesteps].astype(np.float32) * 50.0
            else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Json file with Chinese->English mappings for global tasks and subgoals. If omitted, the script will look for electricmeter_translation_map.json next to itself.",
    )
    parser.add_argument(
        "--use_subgoal_as_task",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Log filename written under output_dir. Set empty string to disable file logging.",
    )
...
def main() -> int:
    ...
    if args.task_name and output_root_dir.exists() and args.overwrite:
    ...
    original_stdout = sys.stdout
    original_stderr = sys.stderr
    log_filename = str(args.log_filename).strip()

    try:
        if log_filename:
            log_path = output_root_dir / log_filename
            log_fp, original_stdout, original_stderr = setup_output_logging(log_path)
        ...
        translation_map, translation_map_path = load_translation_map(args.translation_json)
        print(f"[INFO] Loaded translation map with {len(translation_map)} entries from: {translation_map_path}")
        print(f"[INFO] Ignoring annotation-only quality labels: {sorted(IGNORED_ANNOTATION_LABELS)}")
        ...
        for task_idx, (task_key, traj_list) in enumerate(task_groups.items()):
            ...
            task_instruction = resolve_global_task_instruction(task_key, translation_map)
            dataset_output_dir = output_root_dir if args.task_name else (output_root_dir / task_output_name)

            if dataset_output_dir.exists() and args.overwrite and dataset_output_dir != output_root_dir:
            ...
            subgoal_vocab, subgoal_to_idx = collect_subgoal_vocab_for_trajs(valid_traj_list, ann_index, translation_map)
