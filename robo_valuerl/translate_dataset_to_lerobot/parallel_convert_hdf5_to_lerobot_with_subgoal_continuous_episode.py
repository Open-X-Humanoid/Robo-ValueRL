...
BUILTIN_EN_TEXT_MAP = {
    "<none>": "<none>",
    "Pick up the uninspected joint": "Pick up the uninspected joint",
    "Inspect the joint quality": "Inspect the joint quality",
    "Observe the joint passage": "Observe the joint passage",
    "Place the joint into the good-part pile": "Place the joint into the good-part or defective-part pile",
    "Inspect joint quality and separate good parts from defective parts": "Inspect joint quality and separate good parts from defective parts",
}
...
def load_translation_map(translation_json_path: str | None) -> dict[str, str]:
    mapping = dict(BUILTIN_EN_TEXT_MAP)
    if not translation_json_path:
        return mapping

    path = Path(translation_json_path)
    if not path.exists():
        raise FileNotFoundError(f"translation json does not exist: {translation_json_path}")
...
def translate_to_english(text: str, translation_map: dict[str, str] | None = None) -> str:
    ...
    if not s:
        return s
    mapping = translation_map if translation_map is not None else BUILTIN_EN_TEXT_MAP
    if s in mapping:
        return str(mapping[s])
    normalized_lookup = normalize_text_key(s)
    for k, v in mapping.items():
        if normalize_text_key(k) == normalized_lookup:
            return str(v)
    return s
...
def renumber_successful_results(
    dataset_output_dir: Path,
    results: List[dict],
    chunk_size: int,
) -> List[dict]:
    """
    After conversion finishes, compact successful episodes into a continuous
    episode index space. This keeps parquet/video filenames continuous even if
    some episodes were skipped or deleted during conversion.

    It also rewrites parquet columns and episode_stats so internal metadata stays
    consistent with the new filenames.
    """
    sorted_results = sorted(results, key=lambda x: int(x["episode_index"]))
    new_results: List[dict] = []
    running_global_index = 0

    for new_episode_index, r in enumerate(sorted_results):
        old_parquet_rel = Path(r["parquet_path"])
        old_video_rels = [Path(v) for v in r.get("video_paths", [])]

        old_parquet_abs = dataset_output_dir / old_parquet_rel
        old_video_abs_list = [dataset_output_dir / v for v in old_video_rels]

        new_chunk_index = new_episode_index // max(int(chunk_size), 1)
        new_episode_stem = f"episode_{new_episode_index:06d}"

        table = pq.read_table(old_parquet_abs)
        columns = table.to_pydict()

        length = int(r["length"])
        columns["episode_index"] = [int(new_episode_index)] * length
        columns["index"] = list(range(running_global_index, running_global_index + length))

        new_parquet_rel = Path("data") / f"chunk-{new_chunk_index:03d}" / f"{new_episode_stem}.parquet"
        new_parquet_abs = dataset_output_dir / new_parquet_rel
        ensure_dir(new_parquet_abs.parent)

        tmp_parquet_abs = new_parquet_abs.with_suffix(".tmp.parquet")
        write_episode_parquet(tmp_parquet_abs, columns)

        if old_parquet_abs.resolve() != new_parquet_abs.resolve():
            safe_unlink(old_parquet_abs)
        tmp_parquet_abs.replace(new_parquet_abs)

        new_video_rels: List[str] = []
        for old_video_rel, old_video_abs in zip(old_video_rels, old_video_abs_list):
            parts = old_video_rel.parts
            if len(parts) < 4:
                raise ValueError(f"Unexpected video path format: {old_video_rel}")
            video_key = parts[2]

            new_video_rel = Path("videos") / f"chunk-{new_chunk_index:03d}" / video_key / f"{new_episode_stem}.mp4"
            new_video_abs = dataset_output_dir / new_video_rel
            ensure_dir(new_video_abs.parent)

            if old_video_abs.exists() and old_video_abs.resolve() != new_video_abs.resolve():
                safe_unlink(new_video_abs)
                old_video_abs.replace(new_video_abs)

            new_video_rels.append(str(new_video_rel))

        updated_stats = dict(r.get("episode_stats", {}))
        updated_stats["episode_index"] = _numeric_stats_from_matrix(
            np.asarray(columns["episode_index"], dtype=np.int64)
        )
        updated_stats["index"] = _numeric_stats_from_matrix(
            np.asarray(columns["index"], dtype=np.int64)
        )

        updated_result = dict(r)
        updated_result["episode_index"] = int(new_episode_index)
        updated_result["parquet_path"] = str(new_parquet_rel)
        updated_result["video_paths"] = new_video_rels
        updated_result["episode_stats"] = updated_stats

        new_results.append(updated_result)
        running_global_index += length
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional json file with custom Chinese->English mappings for global tasks and subgoals.",
    )
    parser.add_argument(
        "--use_subgoal_as_task",
        action="store_true",
...
def main() -> int:
    ...
    try:
        ...
        translation_map = load_translation_map(args.translation_json)
        if args.translation_json:
            print(f"[INFO] Loaded translation map with {len(translation_map)} entries from: {args.translation_json}")
        else:
            print(f"[INFO] Using built-in English translation map with {len(translation_map)} entries")
        ...
        for task_output_name, plan in task_plans.items():
            ...
            plan["results"] = renumber_successful_results(
                dataset_output_dir=dataset_output_dir,
