...
def main_worker(local_rank=None, world_size=None, is_ddp=False):
    ...
    transforms = _transforms.compose([
        ...
        _transforms.TokenizePrompt(PaligemmaTokenizerWithQuality(max_len=200, use_quality=True), discrete_state_input=False),
    ...
    # Split train/val indices (3% val set); create separate wrappers so the val set isn't polluted by data augmentation
    total_len = int(len(offline_dataset) * config.sample_from_ratio)
    val_len = max(1, int(total_len * 0.03))
    train_len = total_len - val_len

    all_indices = list(range(len(offline_dataset)))
    rng = np.random.RandomState(42)
    rng.shuffle(all_indices)
    train_indices = all_indices[:train_len]
    val_indices = all_indices[train_len:train_len + val_len]

    # Training set: with image_transforms data augmentation
    train_dataset = XHumanoidLerobotOfflineWrapperForBC(offline_dataset, sample_from_ratio=1.0, transforms=transforms, image_transform=image_transforms)
    train_dataset.sample_indices = train_indices
    train_dataset.sample_len = len(train_indices)

    # Validation set: without image_transforms, to keep evaluation clean
    val_dataset = XHumanoidLerobotOfflineWrapperForBC(offline_dataset, sample_from_ratio=1.0, transforms=transforms, image_transform=None)
    val_dataset.sample_indices = val_indices
    val_dataset.sample_len = len(val_indices)

    if is_main:
        print(f"Dataset split: train={train_len}, val={val_len} ({val_len/total_len*100:.1f}%)")

    sampler = DistributedSampler(train_dataset, num_replicas=world_size, rank=local_rank, shuffle=True) if is_ddp else None

    dataloader = DataLoader(
        train_dataset,
        sampler=sampler,
        shuffle=(sampler is None),  # enable shuffle in single-GPU mode
        batch_size=per_device_batch_size,
        num_workers=4,        # too high a num_workers increases CPU memory pressure, indirectly affecting GPU communication
        pin_memory=True,     # must be used together with non_blocking=True
        drop_last=True
    )

    val_sampler = DistributedSampler(val_dataset, num_replicas=world_size, rank=local_rank, shuffle=False) if is_ddp else None
    ...
    for epoch in range(config.num_epochs):
        ...
        for idx, batch in progress_bar:
            ...
            # F. Validation + save logic (ALL ranks must execute validation to stay in sync)
            if global_step % algo_config.save_interval_steps == 0:
                # --- Validation (multi-GPU: each rank validates its shard, then all-reduce) ---
                agent_model = algorithm.agent.module if is_ddp else algorithm.agent
                agent_model.eval()
                val_losses_sum = {}
                val_count = 0
                with torch.no_grad():
                    for val_batch in val_dataloader:
                        val_batch = recursive_to_device(val_batch, device, non_blocking=True)
                        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):
                            val_loss_dict = agent_model.forward(val_batch)
                        for k, v in val_loss_dict.items():
                            if v is not None:
                                val_losses_sum[k] = val_losses_sum.get(k, 0.0) + v.detach().item()
                        val_count += 1
                        del val_batch, val_loss_dict
                agent_model.train()

                # All-reduce across ranks to get global average
                if is_ddp:
                    keys = sorted(val_losses_sum.keys())
                    vals_tensor = torch.tensor(
                        [val_losses_sum.get(k, 0.0) for k in keys] + [float(val_count)],
                        device=device, dtype=torch.float64,
                    )
                    dist.all_reduce(vals_tensor, op=dist.ReduceOp.SUM)
                    global_val_count = vals_tensor[-1].item()
                    if global_val_count > 0:
                        val_metrics = {f"val/{k}": vals_tensor[i].item() / global_val_count for i, k in enumerate(keys)}
                    else:
                        val_metrics = {}
                else:
                    if val_count > 0:
                        val_metrics = {f"val/{k}": v / val_count for k, v in val_losses_sum.items()}
                    else:
                        val_metrics = {}

                if val_metrics and is_main:
                    val_total = sum(val_metrics.values())
                    val_metrics["val/total_loss"] = val_total
                    wandb.log(val_metrics, step=global_step)
                    print(f"[Step {global_step}] Val loss: {val_total:.4f}")
