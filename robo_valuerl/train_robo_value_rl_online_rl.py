...
from dataset.x_humanoid_lerobot_offline_wrapper import XHumanoidLerobotOfflineWrapperForBC, XHumanoidLerobotOfflineWrapperForBCWithOnlineData
...
@torch.no_grad()
def run_validation(agent, val_loader, device, tag="val", is_ddp=False):
    """Run validation on a dataloader via agent.forward(), return average loss dict.

    Each rank validates its own shard (via DistributedSampler).
    Results are all-reduced across ranks so every rank gets the global average.
    """
    raw_model = agent.module if is_ddp else agent
    ...
    for batch in val_loader:
        ...
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):
            losses = raw_model.forward(batch)
        for k, v in losses.items():
            key = f"{tag}/{k}"
            if v is not None:
                total_metrics[key] = total_metrics.get(key, 0.0) + v.item()
        n_batches += 1
    raw_model.model.train()

    # All-reduce across ranks to get global average
    if is_ddp:
        keys = sorted(total_metrics.keys())
        # Pack [loss_0, loss_1, ..., n_batches] into one tensor for a single all_reduce
        vals_tensor = torch.tensor(
            [total_metrics.get(k, 0.0) for k in keys] + [float(n_batches)],
            device=device, dtype=torch.float64,
        )
        dist.all_reduce(vals_tensor, op=dist.ReduceOp.SUM)
        global_n_batches = vals_tensor[-1].item()
        if global_n_batches == 0:
            return {}
        return {k: vals_tensor[i].item() / global_n_batches for i, k in enumerate(keys)}
    else:
        if n_batches == 0:
            return {}
...
def main_worker(local_rank=None, world_size=None, is_ddp=False):
    ...
    online_rollout_wrapper = XHumanoidLerobotOfflineWrapperForBCWithOnlineData(online_rollout_dataset, sample_from_ratio=1, transforms=transforms, image_transform=image_transforms)

    # --- 5% validation split for both offline and online ---
    val_ratio = 0.05
    def split_train_val(dataset, val_ratio, seed=42):
        n = len(dataset)
    ...
    offline_train, offline_val = split_train_val(filtered_expert_demonstration_wrapper, val_ratio)
    online_train, online_val = split_train_val(online_rollout_wrapper, val_ratio)

    if is_main:
        print(f"[Val Split] offline: train={len(offline_train)}, val={len(offline_val)}")
        print(f"[Val Split] online:  train={len(online_train)}, val={len(online_val)}")

    combined_dataset = ConcatDataset([offline_train, online_train])
    val_offline_dataset = offline_val
    val_online_dataset = online_val
    ...
    if local_rank == 0:
        dataset_names = ["offline_train", "online_train"]
    ...
    # --- Validation dataloaders (with DistributedSampler for multi-GPU) ---
    val_batch_size = max(per_device_batch_size, 16)
    val_offline_sampler = DistributedSampler(val_offline_dataset, num_replicas=world_size, rank=local_rank, shuffle=False) if is_ddp else None
    val_online_sampler = DistributedSampler(val_online_dataset, num_replicas=world_size, rank=local_rank, shuffle=False) if is_ddp else None
    val_offline_loader = DataLoader(
        val_offline_dataset,
        sampler=val_offline_sampler,
    ...
    val_online_loader = DataLoader(
        val_online_dataset,
        sampler=val_online_sampler,
        batch_size=val_batch_size,
        shuffle=False,
        num_workers=4,
        pin_memory=True,
    ...
    for epoch in range(config.num_epochs):
        ...
        for idx, batch in progress_bar:
            ...
            # --- Periodic validation (ALL ranks must execute to stay in sync) ---
            ...
            if global_step % val_interval == 0 and global_step > 0:
                ...
                val_offline_metrics = run_validation(
                    algorithm.agent, val_offline_loader, device,
                    tag="val_offline", is_ddp=is_ddp,
                )
                val_online_metrics = run_validation(
                    algorithm.agent, val_online_loader, device,
                    tag="val_online", is_ddp=is_ddp,
                )
                if is_main:
                    all_val_metrics = {**val_offline_metrics, **val_online_metrics}
                    wandb.log(all_val_metrics, step=global_step)
                    print(f"[Val] offline: { {k: f'{v:.4f}' for k, v in val_offline_metrics.items()} }")
                    print(f"[Val] online:  { {k: f'{v:.4f}' for k, v in val_online_metrics.items()} }")
            ...
            if is_main_computer and global_step % algo_config.save_interval_steps == 0:
                ...
                if has_adaptive_layer:
                    # Backward-compatible: save adaptive_layer separately
